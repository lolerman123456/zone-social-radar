...
const RadarMap: React.FC = () => {
  ...
  const auth = getAuth();
  const db = getDatabase();  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      try {
        ...
        map.addListener("dragstart", () => setMapDragged(true));
    ...
    initMap();
  }, [location, mapLoaded, radiusFeet, isAnimating]);  useEffect(() => {
  ...
  useEffect(() => {
    ...
    otherUsers.forEach(user => {
      ...
      const userLatLng = new google.maps.LatLng(user.lat, user.lng);
      const myLatLng = new google.maps.LatLng(location.latitude, location.longitude);
      const distance = google.maps.geometry.spherical.computeDistanceBetween(myLatLng, userLatLng);
  ...
  }, [otherUsers, location, radiusMeters, ghostMode]);  useEffect(() => {
    if (!permissionState) {
      requestPermission();
    }
