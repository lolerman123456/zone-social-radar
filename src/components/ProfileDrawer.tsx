...
const ProfileDrawer: React.FC<ProfileDrawerProps> = ({ 
  ...
  signOutButtonClassName = "border-white/10 text-white hover:bg-white/5"
}) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-6" autoComplete="off">
          ...
          <div className="pt-4 space-y-4">
            ...
            <Button 
              type="button" 
              variant="outline"
              className={`w-full flex items-center justify-center ${signOutButtonClassName}`}
              onClick={handleSignOut}
            >
              <LogOut className="h-4 w-4 mr-2" /> 
              <span>{signOutButtonLabel}</span>
            </Button>
          </div>
        </form>
      </SheetContent>
